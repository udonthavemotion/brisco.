...
async function populateStuds() {
  try {
    console.log('Starting to populate stud data...');
    
    for (const stud of studData) {
      console.log(`Adding ${stud.name}...`);
      
      const response = await axios.post('http://localhost:1337/api/studs', {
        data: {
          ...stud,
    ...
    console.log('All studs have been added successfully!');
    
  } catch (error) {
    console.error('Error populating studs:', error.response?.data || error.message);
  }
}
