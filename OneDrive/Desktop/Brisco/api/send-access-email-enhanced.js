...
function logError(context, error, additionalInfo = {}) {
  console.error(`[BRISC API ERROR - ${context}]`, {
...
try {
  ...
  if (!resendKey) {
    logError('INIT', new Error('RESEND_API_KEY environment variable is missing'));
  } else {
    resend = new Resend(resendKey);
    logInfo('INIT', 'Resend initialized successfully');
  }
} catch (error) {
  logError('INIT', error, { service: 'Resend' });
}
...
try {
  ...
  if (!supabaseUrl || !supabaseServiceKey) {
    logError('INIT', new Error('Supabase environment variables missing'), {
      hasUrl: !!supabaseUrl,
      hasServiceKey: !!supabaseServiceKey
    });
  } else {
    supabase = createClient(supabaseUrl, supabaseServiceKey);
    logInfo('INIT', 'Supabase initialized successfully');
  }
} catch (error) {
  logError('INIT', error, { service: 'Supabase' });
}
...
export default async function handler(req, res) {
  ...
  logInfo('REQUEST_START', 'Processing request', {
    requestId,
    method: req.method,
  ...
  const allowedOrigins = [
    ...
    'https://brisco.vercel.app',
    'https://brisco-streetwear.vercel.app'
  ];
  ...
  if (req.method === 'OPTIONS') {
    logInfo('PREFLIGHT', 'Handling OPTIONS request', { requestId });
    return res.status(200).end();
  }

  // Only allow POST requests
  if (req.method !== 'POST') {
    logError('METHOD_NOT_ALLOWED', new Error(`Method ${req.method} not allowed`), { requestId });
    return res.status(405).json({ error: 'Method not allowed' });
  }

  // Validate services are initialized
  if (!resend) {
    logError('SERVICE_UNAVAILABLE', new Error('Resend service not initialized'), { requestId });
  ...
  if (!email) {
    logError('VALIDATION', new Error('Email is required'), { requestId });
    return res.status(400).json({ error: 'Email is required' });
  }

  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(email)) {
    logError('VALIDATION', new Error('Invalid email format'), { requestId, email });
    return res.status(400).json({ error: 'Invalid email format' });
  }

  try {
    logInfo('PROCESSING', 'Processing access request', { requestId, email });
    ...
    if (supabase) {
      try {
        ...
        logInfo('LEAD_CAPTURE', 'Attempting to capture lead', { requestId, leadData });

        const { data: leadResult, error: supabaseError } = await supabase
          .from('leads')
          .insert([leadData])
          .select();
        ...
        if (supabaseError) {
          ...
          if (supabaseError.code === '23505') {
            logInfo('LEAD_CAPTURE', 'Email already exists in leads table', { requestId, email });
            leadCaptured = true; // Still consider it captured
          } else {
            logError('LEAD_CAPTURE', supabaseError, { requestId, leadData });
            leadError = supabaseError.message;
            // Don't fail the email send if lead capture fails
          }
        } else {
          logInfo('LEAD_CAPTURE', 'Lead captured successfully', { requestId, leadResult: leadResult[0] });
          leadCaptured = true;
        }
      } catch (leadCaptureError) {
        logError('LEAD_CAPTURE', leadCaptureError, { requestId });
        leadError = leadCaptureError.message;
        // Continue with email send even if lead capture fails
      }
    } else {
      logInfo('LEAD_CAPTURE', 'Supabase not configured, skipping lead capture', { requestId });
    }

    // Step 2: Send access email via Resend
    logInfo('EMAIL_SEND', 'Sending access email', { requestId, email });
...
Born in Thibodaux • Built for Leaders • Worn by Legends
    ...
    if (emailResult.error) {
      logError('EMAIL_SEND', emailResult.error, { requestId });
    ...
    logInfo('SUCCESS', 'Request completed successfully', { 
      requestId, 
    ...
    const processingTime = Date.now() - startTime;
    logError('GENERAL_ERROR', error, { requestId, processingTime: `${processingTime}ms` });
